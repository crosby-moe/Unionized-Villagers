...
public class StrikeTaskList {
    public static ImmutableList<@NotNull Pair<Integer, ? extends Task<? super VillagerEntity>>> createStrikeTasks(RegistryEntry<VillagerProfession> profession, float speed) {
        return ImmutableList.of(
            Pair.of(0, StrikingTasks.createStop()),
            Pair.of(
                2,
                Tasks.pickRandomly(ImmutableList.of(Pair.of(GoAroundTask.create(MemoryModuleType.MEETING_POINT, 0.4F, 40), 2), Pair.of(MeetVillagerTask.create(), 2)))
            ),
